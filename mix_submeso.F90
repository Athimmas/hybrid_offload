...
!-----------------------------------------------------------------------
   ...
   do k=1,km
    ...
    !$OMP PARALLEL DO SHARED(CONTINUE_INTEGRAL,BX_VERT_AVG,RX,RY,ML_DEPTH)PRIVATE(i,WORK3)num_threads(30)SCHEDULE(dynamic,16)
    do j=1,ny_block
        do i=1,nx_block
...
!-----------------------------------------------------------------------
   ...
   if ( luse_const_horiz_len_scale ) then
    ...
    do j=1,ny_block
     ...
     do k=2,km
        !$OMP PARALLEL DO SHARED(WORK3,CONTINUE_INTEGRAL,WORK2,k,bid,dzw,zt,dzwr,RZ_SAVE,ML_DEPTH)PRIVATE(i,j)DEFAULT(NONE)num_threads(30)
        do j=1,ny_block
           do i=1,nx_block
...
!-----------------------------------------------------------------------
   ...
   do k=1,km
     ...
     do kk=ktp,kbt
       ...
       !$OMP SHARED(BX_VERT_AVG,BY_VERT_AVG,DXT,DYT,max_hor_grid_scale,efficiency_factor,kk,k,bid)num_threads(30)  
       do j=1,ny_block
           do i=1,nx_block
